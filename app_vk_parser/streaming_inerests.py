...
def func(batch_df, batch_id):  
    ...
    for data in batch_df.toJSON().collect():
        data = eval(data)
        if data['topic_str'] not in hobbi.keys():
             hobbi[data['topic_str']]=1
        else:
            hobbi[data['topic_str']] += 1
    for key in hobbi:        
        interests_chart.append({'sets':key.split(","), 'size':hobbi[key]})  
    with open('../json/interests_chart.json', 'w') as f:
        json.dump({'interest':interests_chart}, f)
    print ({'interest':interests_chart})
    
df_hobbi = spark \
    .readStream \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "kafka-svc:9092") \
    .option("subscribePattern", "hobbi.*") \
    .load()

hobbies = df_hobbi.selectExpr("CAST(value AS STRING)", "CAST(key AS STRING)", "CAST(topic AS STRING)")

hobbies = hobbies.groupBy("value").agg(F.collect_set("topic")).withColumnRenamed('collect_set(topic)', 'topic')\
                                                              .withColumn('topic_str',F.array_join(F.col('topic'),','))\
                                                              .select('value','topic_str')


hobbies_q = hobbies.writeStream.trigger(processingTime="1 minutes").outputMode("update").foreachBatch(func).start()
hobbies_q.awaitTermination()
